...
def generate_and_download_document(data):
    ...
    action_text = get_action_text(data.get('action'), data.get('other_action', '').strip())
    ...
    context = {
        ...
        'name_IP': data.get('name_ip_or_too', '') if not is_too_template else '',
        'name_TOO': data.get('name_ip_or_too', '') if is_too_template else '',
    ...
    name_ip_or_too = data.get('name_ip_or_too', '').replace('"', '').replace('/', '').replace('\\', '')  # Удаление потенциально проблемных символов
...
def determine_entity_type_and_name(initial_data):
    ...
    Возвращает флаг is_too_template и очищенное название name_ip_or_too_value.
    """
    if initial_data.get('iin_or_bin', '')[4] in ('4', '5'):
        is_too_template = True
        name_ip_or_too_value = extract_title_too(initial_data.get('name_ip_or_too', ''))
    else:
        is_too_template = False
        name_ip_or_too_value = extract_title_ip(initial_data.get('name_ip_or_too', ''))
...
def get_deistvuushego(iin_or_bin):
    ...
    gender = 'masc' if gender_digit in ('1', '3', '5', '0') else 'femn'
...
def get_document_template_and_path(iin_or_bin):
    ...
    template_path = os.path.join(settings.BASE_DIR, 'agreements_templates', template_filename)
...
def get_formatted_date():
    ...
    formatted_date = now.strftime(f"«%d» {month_ru} %Y года").lstrip("0").replace(" 0", " ")
...
def inflect_fio_in_genitive(full_name):
    ...
    response = requests.get(f"https://ws3.morpher.ru/russian/declension?s={encoded_name}", headers=headers)
...
def extract_title_too(original_string):
    # Удаление "ТОО " из начала строки, если оно присутствует, включая кавычки вокруг ТОО
    cleaned_string = re.sub(r'^"ТОО\s+(.*?)"$', r'\1', original_string)
...
def format_location(location):
    ...
    abbreviations = ['Д\.', 'Мкр\.', 'Ул\.', 'Пр\.', 'С\.О\.', 'Н\.П\.', 'Зд\.', 'Ст-Е', 'Кв\.']
    for abbr in abbreviations:
        location = re.sub(fr'\b{abbr}', lambda x: x.group(0).lower(), location, flags=re.IGNORECASE)

    # 3. Сокращение "ГОРОД" до "г." и обработка "Г." в нижнем регистре с пробелом после точки
    ...
    # Приведение первого символа каждого слова к верхнему регистру, кроме специальных слов и аббревиатур
    location = ' '.join(word.title() if not re.match(r'^(г\.|д\.|мкр\.|ул\.|пр\.|с\.о\.|н\.п\.|зд\.|ст-е|кв\.)$', word.lower()) else word for word in location.split())
...
def fio_to_initials(full_name):
    ...
    if len(name_parts) == 3:  # Фамилия Имя Отчество
        initials = f"{name_parts[0]} {name_parts[1][0]}.{name_parts[2][0]}."
